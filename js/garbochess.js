...
function NNUE() {
    ...
    this.biasFeature = new Array(this.FT_OUTPUT_DIM).fill(0);
    
    // Output layer weights
    this.weightsOutput = this.initializeWeights(this.FT_OUTPUT_DIM * 2, 1);
    this.biasOutput = 0;
    
    // Accumulators for incremental updates [white, black]
    this.accumulator = [
        new Array(this.FT_OUTPUT_DIM).fill(0),
        new Array(this.FT_OUTPUT_DIM).fill(0)
    ];
}

NNUE.prototype.initializeWeights = function(rows, cols) {
    var scale = Math.sqrt(2.0 / (rows + cols));
    var weights = new Array(rows * cols);
...
NNUE.prototype.getFeatureIndex = function(piece, square, perspective) {
    var pieceType = piece & 0x7;
    var pieceColor = (piece & 0x8) ? 1 : 0;
    
    var adjustedPiece = pieceType - 1;
    if (pieceColor !== perspective) {
        adjustedPiece += 6;
    }
    
    var adjustedSquare = square;
    if (perspective === 1) {
        adjustedSquare = square ^ 56;
    }
    
    return adjustedPiece * 64 + adjustedSquare;
};

NNUE.prototype.refreshAccumulator = function(board, sideToMove) {
    for (var p = 0; p < 2; p++) {
        for (var i = 0; i < this.FT_OUTPUT_DIM; i++) {
            this.accumulator[p][i] = this.biasFeature[i];
        }
    }
    ...
    for (var sq = 0; sq < 256; sq++) {
        ...
        if (piece & 0x18) {
            for (var p = 0; p < 2; p++) {
                var idx = this.getFeatureIndex(piece, sq, p);
...
NNUE.prototype.addFeatureToAccumulator = function(perspective, featureIdx) {
    var offset = featureIdx * this.FT_OUTPUT_DIM;
    for (var i = 0; i < this.FT_OUTPUT_DIM; i++) {
        this.accumulator[perspective][i] += this.weightsFeature[offset + i];
    }
};

NNUE.prototype.removeFeatureFromAccumulator = function(perspective, featureIdx) {
    var offset = featureIdx * this.FT_OUTPUT_DIM;
    for (var i = 0; i < this.FT_OUTPUT_DIM; i++) {
        this.accumulator[perspective][i] -= this.weightsFeature[offset + i];
    }
...
NNUE.prototype.evaluate = function(sideToMove) {
    var us = sideToMove ? 1 : 0;
    var them = us ^ 1;
    
    var hidden = new Array(this.FT_OUTPUT_DIM * 2);
    
    for (var i = 0; i < this.FT_OUTPUT_DIM; i++) {
        var val = this.accumulator[us][i];
        val = Math.max(0, Math.min(127, val));
        hidden[i] = val;
    }
    
    for (var i = 0; i < this.FT_OUTPUT_DIM; i++) {
        var val = this.accumulator[them][i];
        val = Math.max(0, Math.min(127, val));
        hidden[this.FT_OUTPUT_DIM + i] = val;
    }
    
    var output = this.biasOutput;
    for (var i = 0; i < hidden.length; i++) {
        output += hidden[i] * this.weightsOutput[i];
    }
    
    return Math.round(output * 400);
};
...
function TTEntry(lock, value, flags, depth, move) {
    this.lock = lock;
    this.value = value;
    this.flags = flags;
    this.depth = depth;
...
function TranspositionTable(sizeMB) {
...
TranspositionTable.prototype.clear = function() {
    for (var i = 0; i < this.size; i++) {
        this.table[i] = null;
    }
    this.currentAge++;
...
TranspositionTable.prototype.probe = function(key) {
    var idx = key & (this.size - 1);
    var entry = this.table[idx];
    
    if (entry !== null && entry.lock === key) {
        this.hits++;
        return entry;
    }
...
TranspositionTable.prototype.store = function(key, move, score, depth, flag) {
    var idx = key & (this.size - 1);
    this.table[idx] = new TTEntry(key, score, flag, depth, move);
};
...
function EvaluateNNUE() {
    if (!g_nnue) return g_baseEval;
    return g_nnue.evaluate(g_toMove ? 1 : 0);
}

// =============================================================================
// ORIGINAL GARBOCHESS CODE (Preserved for compatibility)
// =============================================================================
...
var g_lmrEnabled = true;
var g_pvsEnabled = true;
var g_nullMoveEnabled = true;
var g_aspirationWindow = 50;
...
function Search(finishMoveCallback, maxPly, finishPlyCallback) {
    ...
    for (i = 1; i <= maxPly && g_searchValid; i++) {
        ...
        if (i > 4 && g_aspirationWindow > 0) {
            alpha = value - g_aspirationWindow;
            beta = value + g_aspirationWindow;
        } else {
            alpha = minEval;
            beta = maxEval;
        }
        
        var tmp = AlphaBeta(i, 0, alpha, beta, true);
        
        // Research if outside aspiration window
        if (g_aspirationWindow > 0 && i > 4 && (tmp <= alpha || tmp >= beta)) {
            alpha = minEval;
            beta = maxEval;
            tmp = AlphaBeta(i, 0, alpha, beta, true);
        }
...
function Evaluate() {
    // Use NNUE if available, otherwise fall back to classical eval
    if (g_nnue && g_nnueReady) {
        var nnueScore = EvaluateNNUE();
        // Blend NNUE with classical eval for stability
        var classicalScore = EvaluateClassical();
        return Math.round(0.7 * nnueScore + 0.3 * classicalScore);
    }
...
function StoreHash(value, flags, ply, move, depth) {
	...
	// Also store in large TT if available
	if (g_tt) {
	    g_tt.store(g_hashKeyLow, move, value, ply, flags);
	}
}
...
function AllCutNode(ply, depth, beta, allowNull) {
    ...
    for (;;) {
        ...
        if (g_inCheck) {
            ...
            if (g_lmrEnabled && movePicker.stage == 5 && movePicker.atMove > 5 && ply >= 3) {
...
function AlphaBeta(ply, depth, alpha, beta, isPV) {
    if (ply <= 0) {
    ...
    for (;;) {
        ...
        if (g_pvsEnabled && moveMade) {
            // Zero-window search for non-PV nodes
            value = -AllCutNode(plyToSearch, depth + 1, -alpha, true);
            if (value > alpha && value < beta) {
                // Full window search if promising
                value = -AlphaBeta(plyToSearch, depth + 1, -beta, -alpha, true);
            }
        } else {
            value = -AlphaBeta(plyToSearch, depth + 1, -beta, -alpha, isPV);
        }
...
// Global flag for NNUE readiness
var g_nnueReady = false;
...
function ResetGame() {
    ...
    // Initialize NNUE and TT
    g_nnue = new NNUE();
    g_nnue.refreshAccumulator(g_board, g_toMove ? 1 : 0);
    g_nnueReady = true;
    
    g_tt = new TranspositionTable(256);
}
...
function MakeMove(move){
    ...
    if (g_nnue && g_nnueReady) {
        // Simplified update - full refresh for now to ensure correctness
        g_nnue.refreshAccumulator(g_board, g_toMove ? 1 : 0);
    }
